...
assistant = client.beta.assistants.create(
    ...
    instructions=f"You are a helpful tutor who helps students understand and solve the Robot Painting Problem. In this problem, a robot must paint a ceiling and a ladder while adhering to specific constraints on movement and painting order. You have access to two functions: 'apply_operator' and 'create_plan'. You will use these functions to help students find solutions to the problem by simulating the actions of the robot and generating plans to achieve specific goals. Do not attempt to solve the problem on your own; always use the functions provided to simulate the robot's actions or generate plans. Here is a TMK representation to help you understand the relationships between the functions you have access to: {tmk_str}. Be as concise as possible. Just use a tool call and describe the output in a succinct manner.",
    model="gpt-4o",
    tools=[
        {
            "type": "function",
